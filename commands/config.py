...
class Config(commands.Cog):
    ...
    @app_commands.command(
        name="setup", description="Set up a channel for Question of the Day"
    )
    ...
    async def setup(
        ...
        try:
            ...
            await interaction.response.send_message(
                "Invalid time format. Please use 24-hour format (HH:MM)", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="linkpack", description="Link a question pack to a channel"
    )
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.describe(
    ...
    async def linkpack(
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            try:
                ...
                await interaction.response.send_message(
                    f"This pack is already linked to {channel.mention}", ephemeral=True
                )

    @app_commands.command(
        name="unlinkpack", description="Unlink a question pack from a channel"
    )
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.describe(
    ...
    @app_commands.command(
        name="listchannels",
        description="List all channels configured for Question of the Day",
    )
    @app_commands.checks.has_permissions(manage_channels=True)
    ...
    @app_commands.command(
        name="sendnow", description="Send a question immediately to a channel"
    )
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.describe(channel="The channel to send a question to")
    async def sendnow(
        ...
        await interaction.followup.send(
            f"Successfully sent a question to {channel.mention}", ephemeral=True
        )
