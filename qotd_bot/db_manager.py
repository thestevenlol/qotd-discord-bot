...
# This ensures db_manager.py can be run standalone for init if needed
load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), ".env"))

# Get DB filename from .env, default to "questions.db"
DATABASE_FILE = os.getenv("DATABASE_FILE", "questions.db")

# Construct DATABASE_PATH relative to this db_manager.py file
# It will place the DB in a 'data' subdirectory next to db_manager.py


DATABASE_PATH = os.path.join(os.path.dirname(__file__), "data", DATABASE_FILE)
...
def get_all_guild_configs() -> List[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM guild_configs WHERE channel_id IS NOT NULL AND send_time IS NOT NULL AND frequency != 'disabled'"
    )
...
def create_pack(guild_id: int, pack_name: str) -> bool:
    ...
    try:
        cursor.execute(
            "INSERT INTO question_packs (guild_id, pack_name) VALUES (?, ?)",
            (guild_id, pack_name),
        )
        conn.commit()
        return True
...
def get_pack(guild_id: int, pack_name: str) -> Optional[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM question_packs WHERE guild_id = ? AND pack_name = ?",
        (guild_id, pack_name),
    )
...
def get_guild_packs(guild_id: int) -> List[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM question_packs WHERE guild_id = ? ORDER BY pack_name",
        (guild_id,),
    )
...
def get_questions_for_pack(pack_id: int) -> List[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM questions WHERE pack_id = ? ORDER BY question_id", (pack_id,)
    )
...
def get_sent_question_ids(guild_id: int, pack_id: int) -> List[int]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT question_id FROM sent_questions WHERE guild_id = ? AND pack_id = ?",
        (guild_id, pack_id),
    )
...
def mark_question_as_sent(guild_id: int, pack_id: int, question_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO sent_questions (guild_id, pack_id, question_id) VALUES (?, ?, ?)",
...
def reset_sent_questions_for_pack(guild_id: int, pack_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM sent_questions WHERE guild_id = ? AND pack_id = ?",
        (guild_id, pack_id),
    )
    conn.commit()
    conn.close()
...
def add_suggestion(guild_id: int, user_id: int, question_text: str) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO suggested_questions (guild_id, user_id, question_text) VALUES (?, ?, ?)",
...
def get_pending_suggestions(guild_id: int) -> List[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM suggested_questions WHERE guild_id = ? AND status = 'pending' ORDER BY suggestion_id",
        (guild_id,),
    )
...
def get_suggestion(suggestion_id: int) -> Optional[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM suggested_questions WHERE suggestion_id = ?", (suggestion_id,)
    )
...
def update_suggestion_status(suggestion_id: int, status: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE suggested_questions SET status = ? WHERE suggestion_id = ?",
...
if __name__ == '__main__':
    ...
    initialize_db() # initialize_db itself uses get_db_connection which now uses DATABASE_PATH
